...
public class BitStringListIntersector extends MetaProblem implements IProblem
{
  ...
  /*public BitStringListIntersector(IBitStringList T,IBitStringList A,IBitStringList B) throws Exception
  {
    ...
    for(int i=0;i<A.size();i++)
    {
      ...
      problem=new Conjunction(
      		...
      		{
      				problem,
      				new Disjunction(notInT,new Conjunction(inA,inB)),
          		new Disjunction(inT,new Disjunction(notInA,notInB))
      		});
    }
    for(int i=0;i<B.size();i++)
    {
