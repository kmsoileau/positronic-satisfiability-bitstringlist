...
public class BitStringListUnioner extends MetaProblem implements IProblem
{
  private static final long serialVersionUID = 1L;
  
  public BitStringListUnioner(IBitStringList union,IBitStringList A,IBitStringList B) throws Exception
  {
    ...
    {
      IProblem inT=new BitStringListMembershipper(A.getBitString(i),union);
      problem=new Conjunction(problem,inT);
    }
    for(int i=0;i<B.size();i++)
    {
      IProblem inT=new BitStringListMembershipper(B.getBitString(i),union);
      problem=new Conjunction(problem,inT);
    }
  ...
  /*public BitStringListUnioner(IBitStringList T,IBitStringList A,IBitStringList B) throws Exception
  {
    ...
    for(int i=0;i<A.size();i++)
    {
      ...
      problem=new Conjunction(problem,
        new Disjunction(notInT,inA,inB),
          new Disjunction(new Conjunction(notInA,notInB),inT));
    }
    for(int i=0;i<B.size();i++)
    {
      IBitString curr=B.getBitString(i);
      IProblem p1=new BitStringListNonmembershipper(curr,T);
      IProblem p2=new BitStringListMembershipper(curr,A);
      IProblem p3=new BitStringListMembershipper(curr,B);
      IProblem p4=new BitStringListNonmembershipper(curr,A);
      IProblem p5=new BitStringListNonmembershipper(curr,B);
      IProblem p6=new BitStringListMembershipper(curr,T);
      problem=new Conjunction(problem,
        new Disjunction(p1,p2,p3),
          new Disjunction(new Conjunction(p4,p5),p6));
    }
    this.setClauses(problem.getClauses());
  }*/
}